...
/**
 * Write Codex home files: config.toml (with MCP server) + AGENTS.md.
 * Codex uses TOML config format, unlike Claude/Copilot's JSON.
 *
 * If `~/.remote-copilot-mcp/codex-base.toml` exists, its contents are used as
 * the base config (allowing the operator to declare additional MCP servers).
 * The sensorium-mcp entry is always appended/overwritten at the end.
 */
function writeCodexHomeFiles(codexHome: string, port: number, secret: string | null): void {
  mkdirSync(codexHome, { recursive: true });

  // Start from user-provided base config if present, otherwise empty.
  let toml = existsSync(CODEX_BASE_CONFIG_PATH)
    ? readFileSync(CODEX_BASE_CONFIG_PATH, "utf-8").trimEnd() + "\n"
    : `# Auto-generated by sensorium-mcp for Codex thread spawning\n`;

  // Always append/overwrite the sensorium-mcp entry so it reflects the current port/secret.
  toml += `\n# Auto-injected by sensorium-mcp — do not edit this section manually\n`;
  toml += `[mcp_servers.sensorium-mcp]\n`;
  toml += `url = "http://127.0.0.1:${port}/mcp"\n`;
  if (secret) {
    toml += `http_headers = { Authorization = "Bearer ${secret}" }\n`;
  }

  writeFileSync(join(codexHome, CODEX_CONFIG_FILENAME), toml, "utf-8");
  writeFileSync(join(codexHome, CODEX_INSTRUCTIONS_FILENAME), CODEX_SYSTEM_PROMPT, "utf-8");
}
