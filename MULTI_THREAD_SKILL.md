...
| Task | Worker Thread | Pattern | Why |
|------|--------------|---------|-----|
| Code review | "Code Review" thread | Delegate-and-Wait | Keeps review context separate; you need findings to act on |
| Dashboard changes | "Dashboard" thread | Delegate-and-Wait | Accumulates dashboard-specific knowledge |
| Refactoring | "Refactoring" thread | Delegate-and-Wait | Large changes stay isolated; you need to verify results |
| Testing | "Testing" thread | Delegate-and-Wait | Test infrastructure knowledge persists |
| Scheduled improvements | "Improvements" thread | Fire-and-Forget | Background tasks don't pollute main context |
| Log cleanup | "Maintenance" thread | Fire-and-Forget | Housekeeping that doesn't affect current work |
| Metrics collection | "Monitoring" thread | Fire-and-Forget | Ongoing telemetry runs independently |
...
### Benefits vs Subagents

| | Subagents | Worker Threads |
|---|---|---|
| Context impact | Returns output to caller, bloats context | Results stay in the worker; only summary comes back |
| Memory | Stateless — no persistence | Persistent — builds domain memory over time |
| Parallel work | Yes | Yes (multiple threads run concurrently) |
| Best for | One-off tasks, quick reads/searches | Ongoing domains, recurring work types |
